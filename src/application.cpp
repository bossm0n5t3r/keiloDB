...
result_t application::create_database(const std::string& database_name) {
  ...
  if (hashed_databases[database_name]) return result_t::already_exist;
...
result_t application::add_database(const database& database) {
  ...
  if (hashed_databases[database.get_name()]) return result_t::already_exist;
...
result_t application::drop_database(const std::string& database_name) {
  ...
  if (!hashed_databases[database_name]) return result_t::cannot_find;

  const auto it = std::find_if(databases.begin(), databases.end(),
                               [database_name](const database& database) {
...
database* application::select_database(const std::string& database_name) {
  ...
  if (!hashed_databases[database_name]) return nullptr;
...
result_t application::import_file(const std::string& file_name) {
  if (file_name.find("json") == std::string::npos) return result_t::cannot_find;
  ...
  if (!file) return result_t::cannot_find;
...
result_t application::export_database(const std::string& database_name,

...
database::database(std::string database_name)
    : name(std::move(database_name)) {}

database::database(std::ifstream& file) { parse_file(file); }

database::database(const database& other) {
  this->name = other.name;
  for (const auto& table : other.tables) this->tables.emplace_back(table);
}

result_t database::create_table(const std::string& table_name,
                                const std::string& key) {
  std::unique_lock<std::mutex> lock{this->mutex};

  if (hashed_tables[table_name]) return result_t::already_exist;
...
result_t database::add_table(const table& other) {
  ...
  if (hashed_tables[other.get_name()]) return result_t::already_exist;
...
result_t database::drop_table(const std::string& table_name) {
  ...
  if (!hashed_tables[table_name]) return result_t::cannot_find;
...
table* database::select_table(const std::string& table_name) {
  ...
  if (!hashed_tables[table_name]) return nullptr;
...
void database::parse_file(std::ifstream& file) {
  ...
  for (auto db = js.cbegin(); db != js.cend(); ++db) {
    ...
    for (const auto& tb : *db) {
      ...
      mutex.lock();
      tables.emplace_back(tb["name"].get<std::string>(),
                          tb["key"].get<std::string>(), records);
      mutex.unlock();
    }
  }
}
